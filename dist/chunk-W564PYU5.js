...
function questionsFor(id, contextIds = []) {
  const questions = {};
  const scope = contextIds.length ? `Judge only the message with id "${id}" in the state. The messages ${contextIds.map((c) => `"${c}"`).join(
    " and "
  )} are earlier prompts from the same conversation, given as context: anything they already state counts as known to the reader of "${id}", but they are not themselves being judged.` : `Consider only the message with id "${id}" in the state.`;
