...
function sessionContext(config, at) {
  ...
  const safe = (text) => applyPrivacy(clampPrompt(text), config.privacy).text;
  if (sessionRepliesEnabled() && at.transcriptPath) {
    try {
      const conversation = recentTurns(at.transcriptPath, at.promptKey, CONTEXT_EXCHANGES, config.bypassPrefix).map((turn) => ({ role: turn.role, text: safe(turn.text) })).filter((turn) => turn.text !== null);
