...
function sessionContext(config, at) {
  ...
  try {
    if (sessionRepliesEnabled() && at.transcriptPath) {
      const conversation = recentTurns(at.transcriptPath, at.promptKey, CONTEXT_EXCHANGES, config.bypassPrefix).map((turn) => ({ role: turn.role, text: applyPrivacy(turn.text, config.privacy).text })).filter((turn) => turn.text !== null);
