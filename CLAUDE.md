...
The allow list in that script is empty and should stay empty: test values are
assembled from fragments at runtime so no file holds a contiguous
credential-shaped string. Never silence a scan you have not read, and never
resolve a blocked push by clicking GitHub's "allow this secret" link.
